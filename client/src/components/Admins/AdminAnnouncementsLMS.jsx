...
export default function AdminAnnouncementsLMS() {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        const res = await fetch(API_BASE + '/admin/announcements?type=lms', { credentials: 'include' });
        if (!res.ok) return;
  ...
  async function handleAdd(e) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const json = await res.json();
        // reload list
        const listRes = await fetch(API_BASE + '/admin/announcements?type=lms', { credentials: 'include' });
        if (listRes.ok) {
        ...
        <main className="col-span-1 md:col-span-3">
          <div className="bg-white rounded-lg shadow">
            <div className="p-6 border-b flex items-center justify-between">
              <div>
                <h2 className="text-xl font-semibold">LMS Announcements (Admin)</h2>
                <p className="text-sm text-gray-500">Manage LMS announcements visible to users.</p>
              </div>
              <div>
                <button onClick={() => setShowForm(s => !s)} className="px-3 py-2 bg-blue-600 text-white rounded flex items-center gap-2">
                  <PlusOutlined /> Add LMS Announcement
                </button>
              </div>
            </div>
